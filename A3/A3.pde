...
// Made by Kassian Houben
// Edited and customised by Emily Licovski
...
String audioFileName = "MammaMia.mp3"; 
...
// Setting size() using set variables
void settings() {
...
void setup() {
  ...
  unit = height / 100; // Unit is the base term that determines how things change based on its size 
  strokeWeight(unit / 10.24);
  groundLineY = height * 3/4;
  center = new PVector(width / 2, height * 3/4);
  
  
  //Export - not used in this case but had difficulties removing it
  // Set up .mp4 export
  if (export) {
...
float getGroundY(float groundX) {
  ...
  float angle = 1 * groundX / unit * 8;

  float groundY = sin(radians(angle + frameCount * 2)) * unit * 1.25 + groundLineY - unit * 0.5;
