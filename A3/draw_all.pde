  ...
  for (int angle = 0; angle <= 270; angle++) {
    ...
    if (sum[0] != 0) {
      if (angle >= 0 && angle <= 15) {
        ...
        extendingSphereLinesRadius = map(sum[90 - round(map((angle - 45), 0, 30, 65, 80))], 0, 40, extendingSphereLinesRadius - extendingSphereLinesRadius / 8, extendingLinesMax * 1.5); // Bass
      }
      
      else if (angle > 90 && angle <= 180) {
        extendingSphereLinesRadius = map(sum[0 + round(map((angle - 90), 0, 30, 0, 15))], 0, 40, extendingSphereLinesRadius - extendingSphereLinesRadius / 8, extendingLinesMax * 1.5); // Bass
      }
